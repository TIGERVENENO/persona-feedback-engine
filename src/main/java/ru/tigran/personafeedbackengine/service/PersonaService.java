...
public class PersonaService {
    ...
    /**
     * Batch persona generation with structured PersonaPromptBuilder.
     *
     * RECOMMENDED approach for batch generation:
     * - Uses PersonaPromptBuilder for optimized system and user prompts
     * - Automatic age distribution calculation across specified range
     * - Clear demographic and psychographic sections in prompt
     * - Robust retry mechanism (5 attempts with exponential backoff)
     *
     * Compared to startBatchPersonaGeneration:
     * - ✅ Better structured prompts with demographic/psychographic sections
     * - ✅ Automatic age distribution (ensures full range coverage)
     * - ✅ All parameters from PersonaGenerationRequest properly formatted
     * - ✅ Same robust retry and validation as existing method
     *
     * @param userId User ID
     * @param request PersonaGenerationRequest with demographics and psychographics
     * @return List of created Persona IDs
     */
    @Transactional
    public List<Long> startBatchPersonaGenerationWithPromptBuilder(Long userId, PersonaGenerationRequest request) {
        log.info("Starting BATCH persona generation with PromptBuilder for user {} with count: {}", userId, request.count());
        ...
        // Call AI with PersonaPromptBuilder - guaranteed to return valid JSON or throw exception
        String batchPersonasJson = aiGatewayService.generatePersonasFromRequest(userId, request);
        ...
        log.info("Successfully received {} personas from AI with PromptBuilder (requested {})", personasData.size(), personaCount);
        ...
        for (int i = 0; i < personasData.size(); i++) {
            ...
            Persona saved = personaRepository.save(persona);
            createdPersonas.add(saved);
            log.info("Created persona entity {} from batch with PromptBuilder ({}/{}): {}",
                    saved.getId(), i + 1, personasData.size(), personaData.name());
        }

        // Flush all personas to database
        personaRepository.flush();
        log.info("Flushed {} personas from batch generation with PromptBuilder to database", createdPersonas.size());

        List<Long> personaIds = createdPersonas.stream().map(Persona::getId).collect(Collectors.toList());
        log.info("Batch persona generation with PromptBuilder completed successfully. IDs: {}", personaIds);
