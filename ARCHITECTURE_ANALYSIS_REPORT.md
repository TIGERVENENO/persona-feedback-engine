...
**Статус исправлений:**
- ✅ **Исправлено:** 4 критические проблемы (Волна 1)
- 🔴 **Ещё нужно:** 8 критических проблем
...
**Недавние исправления (Волна 1):**
- ✅ #2: Некорректная конфигурация Redis TTL (коммит: 33574dc)
- ✅ #6: NPE при парсинге AI ответа (коммит: 5a339c3)
- ✅ #4: Валидация статуса Persona (коммит: ef67805)
- ✅ #12: Проверка пустых списков в DTOs (коммит: 0d70b8c)

---

## 🔴 КРИТИЧЕСКИЕ ПРОБЛЕМЫ (требуют немедленного исправления)

### 1. **Race Condition в обновлении статуса FeedbackSession**
**Файл:** `FeedbackTaskConsumer.java:90-102`

**Проблема:**
```java
long completedCount = feedbackResultRepository.countByFeedbackSessionIdAndStatus(
    session.getId(), FeedbackResult.FeedbackResultStatus.COMPLETED.name()
);
...
**Проблема:** Два отдельных запроса без блокировки. При параллельной обработке несколько consumer'ов могут одновременно проверить условие и обновить сессию несколько раз.

**Последствия:**
- Некорректный статус сессии
- Множественные уведомления пользователю
...
// Использовать атомарный UPDATE с WHERE условием
...
int updateStatusIfNotAlready(@Param("sessionId") Long sessionId,
                              @Param("status") FeedbackSessionStatus status);
```

---

### 2. **N+1 Query Problem при получении результатов сессии**
**[ПЕРЕИМЕНОВАНО из #3 - исправлено #2]**
**Файл:** `FeedbackController.java:95-105`
...
List<FeedbackResultDTO> resultDTOs = results.stream()
    .map(result -> new FeedbackResultDTO(
        ...
        result.getPersona().getId(),      // ❌ LAZY загрузка -> SELECT
        result.getProduct().getId()       // ❌ LAZY загрузка -> SELECT
    ))
    .collect(Collectors.toList());
```

**Последствия:** Для 100 результатов = 1 + 100 + 100 = 201 SQL запрос!

**Решение:**
```java
// В FeedbackResultRepository.java
...
### 3. **Отсутствие идемпотентности в RabbitMQ consumers**
**[ПЕРЕИМЕНОВАНО из #5 - исправлено #4]**
**Файлы:** `PersonaTaskConsumer.java`, `FeedbackTaskConsumer.java`

**Проблема:** При redelivery сообщения (например, после сбоя) consumer обработает его повторно, перезаписывая данные.

**Последствия:**
- Повторная генерация персоны (трата денег на API)
- Перезапись feedback текста
...
public void consumePersonaTask(PersonaGenerationTask task) {
    ...
    if (persona.getStatus() == Persona.PersonaStatus.FAILED) {
        ...
        // Можно решить, retry или skip
    }

    // Продолжаем обработку только если GENERATING
    if (persona.getStatus() != Persona.PersonaStatus.GENERATING) {
        log.warn("Unexpected persona status: {}", persona.getStatus());
        return;
    }
...
### 4. **Отсутствие Dead Letter Queue**
**[ПЕРЕИМЕНОВАНО из #7 - исправлено #6]**
**Файл:** `RabbitMQConfig.java`

**Проблема:** Когда сообщение падает с ошибкой, оно просто теряется (помечается FAILED в БД), но не отправляется в DLQ для анализа.

**Последствия:**
- Невозможно понять причины сбоев
- Нет возможности ручного retry
- Потеря данных о проблемных запросах

**Решение:**
```java
@Bean
public Queue personaGenerationQueue() {
    return QueueBuilder.durable(PERSONA_GENERATION_QUEUE)
        .deadLetterExchange("dlx-exchange")
        .deadLetterRoutingKey("dlx.persona.generation")
        .build();
}

@Bean
public Queue personaDLQ() {
    return new Queue("dlq.persona.generation", true);
}
...
public Binding personaDLQBinding(Queue personaDLQ, DirectExchange dlxExchange) {
    return BindingBuilder.bind(personaDLQ)
        .to(dlxExchange)
        .with("dlx.persona.generation");
}
```

---

### 5. **Глобальное кеширование персон без учета userId**
...
### 6. **Поглощение исключений в consumers**
**[ПЕРЕИМЕНОВАНО из #9]**
...
### 7. **Отсутствие транзакций в controller**
**[ПЕРЕИМЕНОВАНО из #10]**
...
### 9. **Небезопасная аутентификация через X-User-Id**
**[ПЕРЕИМЕНОВАНО из #13]**
...
### 10. **SQL Injection через enum в строковом виде**
**[ПЕРЕИМЕНОВАНО из #14]**
...
### 11. [ПЕРЕИМЕНОВАНО из #15] **Секреты в application.properties**
...
### 12. [ПЕРЕИМЕНОВАНО из #16] **Отсутствие rate limiting**
...
### 13. [ПЕРЕИМЕНОВАНО из #17] **Нет распределенной блокировки для обновления статуса сессии**
...
### 14. [ПЕРЕИМЕНОВАНО из #18] **Отсутствие pagination**
...
### 15. [ПЕРЕИМЕНОВАНО из #19] **Блокирующие HTTP вызовы в consumers**
...
### 16. [ПЕРЕИМЕНОВАНО из #20] **Отсутствие мониторинга и метрик**
...
### 17. [ПЕРЕИМЕНОВАНО из #21] **Отсутствие индексов для часто используемых запросов**
...
### 18. [ПЕРЕИМЕНОВАНО из #22] **Дублирование запросов к БД**
...
### 19. [ПЕРЕИМЕНОВАНО из #23] **Отсутствие кеширования результатов сессий**
...
### 20. [ПЕРЕИМЕНОВАНО из #24] **Отсутствие connection pooling конфигурации**
...
### 21. [ПЕРЕИМЕНОВАНО из #25] **Нет оптимистичной блокировки**
...
### 22. [ПЕРЕИМЕНОВАНО из #26] **CASCADE ALL на User может быть опасным**
...
### 23. [ПЕРЕИМЕНОВАНО из #27] **RESTRICT на Product/Persona блокирует удаление**
...
### 24. [ПЕРЕИМЕНОВАНО из #28] **Кеширование по prompt слишком специфично**
...
### 25. [ПЕРЕИМЕНОВАНО из #29] **Redis password пустой по умолчанию**
...
### 26. [ПЕРЕИМЕНОВАНО из #30] **Retry логика только для 429**
...
### 27. [ПЕРЕИМЕНОВАНО из #31] **Нет различия между retriable и non-retriable ошибками**
...
### 28. [ПЕРЕИМЕНОВАНО из #32] **Нарушение Single Responsibility**
...
### 29. [ПЕРЕИМЕНОВАНО из #33] **Дублирование кода в error handling**
...
### 30. [ПЕРЕИМЕНОВАНО из #34] **Отсутствие DTO validation**
